...
import com.mooga.popularmovies.adapters.TrailerAdapter;
import com.mooga.popularmovies.data.MovieContract;
import com.mooga.popularmovies.models.Movie;
import com.mooga.popularmovies.models.Review;
import com.mooga.popularmovies.models.Trailer;
...
public class DetailActivityFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mImageView = (ImageView) rootView.findViewById(R.id.poster);
        ...
        mTrailersView = (LinearLayout) rootView.findViewById(R.id.trailers_container);
        mReviewsView = (LinearLayout) rootView.findViewById(R.id.reviews_container);

        final ImageButton like  = (ImageButton) rootView.findViewById(R.id.like_button);
        ...
        new AsyncTask<Void, Void, Integer>() {
            ...
            protected void onPostExecute(Integer isFavorited) {
                like.setImageResource(isFavorited == 1 ?
                                ...
                                if (isFavorited == 1) {
                                    ...
                                    new AsyncTask<Void, Void, Integer>() {
                                        ...
                                        protected void onPostExecute(Integer rowsDeleted) {
                                            like.setImageResource(R.drawable.abc_btn_rating_star_off_mtrl_alpha);
                                            if (mToast != null) {
                                                mToast.cancel();
                                            }
                                ...
                                // if it is not in favorites
                                    ...
                                    new AsyncTask<Void, Void, Uri>() {
                                        ...
                                        protected void onPostExecute(Uri returnUri) {
                                            like.setImageResource(R.drawable.abc_btn_rating_star_on_mtrl_alpha);
                                            if (mToast != null) {
                                                mToast.cancel();
                                            }
        ...
        if (mMovie != null) {
            ...
            mVoteAverageView.setText(Double.toString(mMovie.getRating())+"/10");
        }
    ...
    // Reviews create View and Fetch Data
    public class FetchReviewsTask extends AsyncTask<String, Void, List<Review>> {
        ...
        private void setReviewsList(final List<Review> reviews){
            LinearLayout linear = (LinearLayout) mReviewsView.findViewById(R.id.reviews_list_container);
            TextView [] review = new TextView[reviews.size()];
            for(int i = 0; i < reviews.size(); i++) {
                review[i] = new TextView(getActivity());
                review[i].setText("\n" + reviews.get(i).getContent());
                review[i].setTextSize(16f);
                linear.addView(review[i]);
            }
        }
        @Override
    ...
    // Trailers create View and Fetch Data
    public class FetchTrailersTask extends AsyncTask<String, Void, List<Trailer>> {
        ...
        private List<Trailer> getTrailersDataFromJson(String jsonStr) throws JSONException {
            ...
            LinearLayout linear = (LinearLayout) mTrailersView.findViewById(R.id.trailers_list_container);
            Button[] btns = new Button[trailerArray.length()];
        ...
        private void setTrailerList(final List<Trailer> videos){
            LinearLayout linear = (LinearLayout) mTrailersView.findViewById(R.id.trailers_list_container);
            Button[] btns = new Button[videos.size()];
            for(int i = 0; i < videos.size(); i++) {
                btns[i] = new Button(getActivity());
                btns[i].setText("Trailer" + (i + 1));
                btns[i].setHeight(20);
                btns[i].setBackgroundColor(Color.TRANSPARENT);
                btns[i].setCompoundDrawablesWithIntrinsicBounds(R.drawable.play, 0, 0, 0);
                linear.addView(btns[i]);
                final int cur = i;
                btns[i].setOnClickListener(new View.OnClickListener() {
                    public void onClick(View v) {
                        Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://youtube.com/watch?v=" + videos.get(cur)));
